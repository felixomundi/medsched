...
export default function PatientCreateAppointment() {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting, resetForm }) => {
      try {
        ...
        const response = await axiosInstance.post('appointments/create', data, config);
        if (response.data) {
  ...
  const fetchPatients = async(token) => {
    try {     
      setIsLoading(true);
      const response = await axiosInstance.get(`appointments/users`, {
        headers:{
  ...
  useEffect(() => {
    setMinDateTime(getCurrentDateTimeLocal().slice(0, 10)); 
...
<div className="form-group col-md-6">
        ...
        <Select
          ...
          value={patientOptions.find(option => option.value === formik.values.patientId)}
          onChange={option => formik.setFieldValue('patientId', option.value)}
          onBlur={formik.handleBlur('patientId')}
          placeholder="Select Patient"
          className={formik.touched.patientId && formik.errors.patientId ? 'is-invalid' : ''}
        />
        {formik.touched.patientId && formik.errors.patientId ? (
          <div className="invalid-feedback">{formik.errors.patientId}</div>
        ) : null}
      </div>
                  <div className="form-group col-md-6">
              ...
              <div className="card-footer">
                <Link href="/admin/appointments" className="btn btn-secondary float-start">Back</Link>
